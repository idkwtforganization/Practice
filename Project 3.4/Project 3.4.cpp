...
enum type_of_lex {
    ...
    LEX_STRING, LEX_IF, LEX_ELSE, LEX_FALSE, LEX_TRUE, LEX_FOR,
    LEX_FIN,                                                                                    /*18*/
...
Scanner::TW[] = { "", "and", "real", "do",
                 ...
                 "string", "if", "else", "false", "true", "for", NULL};
...
void Parser::O1() {
    int pl0, pl1, pl2, pl3, pl4, tmp_br;
    if (c_type == LEX_ID) {
        check_id();
        ...
        if (c_type == LEX_ASSIGN) {
            ...
            if (c_type == LEX_SEMICOLON || flag_for == true) {
                gl();
            }
            else throw curr_lex;
        }
    ...
    while (index < size) {
        ...
        case LEX_ID:
            ...
            if (TID[i].get_type() == LEX_REAL) {
                if (TID[i].get_assign()) {
                    //                        for(auto l: TID){
                    //                            cout<<l.get_value() << " " << l.get_s_value() << " " << l.get_r_value()<<endl;
                    //                        }
