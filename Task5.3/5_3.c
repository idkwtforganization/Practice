...
void conveyor(char *** mas_of_mas, int count_of_mas, int conveyor_type, int save_in, int save_out, int change_in, int change_out)
{
	int fd[2], i = 0;
	int tmp = dup(0);
	if (conveyor_type == 1){
...
int main(int argc, char *argv[]){
	...
	char *s = NULL, ***mas_of_mas = NULL;
	int save_in = dup(0), save_out = dup(1), change_in = 0, change_out = 0;
	while (s = get_s(fl)){
		int flag_found_word = 0, flag_found_space = 0, count = 0, count_of_mas = 1, conveyor_type = 0;
		...
		conveyor(mas_of_mas, count_of_mas, conveyor_type, save_in, save_out, change_in, change_out);
